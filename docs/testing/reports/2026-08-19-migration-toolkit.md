# Test report · 2026-08-19 · migration toolkit end-to-end
...
- [1. Summary](#1-summary)
- [2. Step 1 — Extract](#2-step-1--extract)
- [3. Step 2 — Route (`classify/route_nodes.py`)](#3-step-2--route-classifyroute_nodespy)
- [4. Step 3 — Load to BigQuery](#4-step-3--load-to-bigquery)
- [5. Step 4 — Transform (Dataform CLI)](#5-step-4--transform-dataform-cli)
- [6. Step 5 — Verify (BigQuery)](#6-step-5--verify-bigquery)
- [7. What was and wasn't tested](#7-what-was-and-wasnt-tested)
- [8. Automatic-migration retest — `migrate/migrate.py`](#8-automatic-migration-retest--migratemigratepy)
- [9. Extractor crash recovery — MySQL checkpoint resume](#9-extractor-crash-recovery--mysql-checkpoint-resume)
- [10. Complex workflow — `sample_flowspec_complex_ecommerce.json`](#10-complex-workflow--sample_flowspec_complex_ecommercejson)
- [11. Gap-fix unit tests — v3.1 guide audit remediation](#11-gap-fix-unit-tests--v31-guide-audit-remediation)

---

## 1. Summary

Rows are numbered by the document section that records them; section 7 has no
row because it records scope/caveats rather than a test result.

| Section | What ran | Result |
| --- | --- | --- |
| 2 · Extract | Pull Alibaba public `banking.txt` + run metadata extractor | **PASS** |
| 3 · Route | `classify/route_nodes.py` on the case-study FlowSpec | **PASS** |
| 4 · Load | Create `dwh.bank_data` in BigQuery, load 41,188 rows | **PASS** |
| 5 · Transform | Dataform CLI compiles + runs `result_table.sqlx` | **PASS** |
| 6 · Verify | Query `result_table`; 8 rows, `SUM(num) = 6,191` | **PASS** |
| 8 · Auto-migration | `migrate/migrate.py all` — `--flowspec`, `--extract-dir`, and `--workflows` selector (generate → compile → run → verify) | **PASS** |
| 8.3 · Workflow selection | `--workflows` selector file — only listed workflows migrated; no-match aborts (exit 1) | **PASS** |
| 9 · Crash recovery | Extractor resume test — workflow- and node-level, append-only checkpoint | **PASS** |
| 10 · Complex workflow | Multi-node workflow — variables resolved, 4 models + 2 decls generated, 3 review items as of the 2026-08-18 run (inherent pipeline limits only), compile PASS | **PASS** |
| 11 · Gap-fix unit tests | As of the 2026-08-19 run: 51 tests — topological sort, cross-cycle deps, cross-workflow deps, partition flagging, verifier expansion, _decl_sqlx fix, docstring fix, e2e generation, translator regression, CSV conversion, CSV overrides, e2e CSV flow | **PASS** |

**Overall: PASS** — data is ready in BigQuery: `$GCP_PROJECT_ID.dwh.result_table`.
[Section 8](#8-automatic-migration-retest--migratemigratepy) records the
automatic-migration retests (including the `--workflows` selector in 8.3) and
[section 9](#9-extractor-crash-recovery--mysql-checkpoint-resume) records the
extractor crash-recovery retest.
[Section 11](#11-gap-fix-unit-tests--v31-guide-audit-remediation) records the
gap-fix unit tests (as of the 2026-08-19 run: 51/51 passed).
...
| gcloud account | `<your-gcloud-account>` (active) |
| GCP project / region | `$GCP_PROJECT_ID` / `asia-southeast2` |
...
| MySQL checkpoint | `dataworks-mysql` container (localhost:3306) available |
...
```bash
bq --project_id=$GCP_PROJECT_ID mk --dataset --location=asia-southeast2 dwh
```
...
```bash
bq --project_id=$GCP_PROJECT_ID load --source_format=CSV --skip_leading_rows=0 --replace \
...
`$WORK_DIR/dataform-e2e` (repo files untouched). Changes for Dataform v3:
- removed deprecated `dataform.json` (cannot coexist with `workflow_settings.yaml`);
- added `dataformCoreVersion: 3.0.64`;
- set `defaultProject: $GCP_PROJECT_ID` in `workflow_settings.yaml` and the
  `bank_data.sqlx` declaration;
- `.df-credentials.json` uses gcloud Application Default Credentials (ADC).

> Note: the `$WORK_DIR/dataform-e2e` scratch project is created outside the
> repository and is **not committed**, so this section is not reproducible as
> written from a clean checkout — the scratch copy plus the v3 edits above must
> be recreated first.

```bash
dataform compile $WORK_DIR/dataform-e2e
dataform run    $WORK_DIR/dataform-e2e
```
...
- The committed `workflow_settings.yaml`/`dataform.json` still carry the
  `my-gcp-project` placeholder — the Dataform run used `$GCP_PROJECT_ID` via the scratch
  project.
...
Env (from `.env`, git-ignored): `GCP_PROJECT_ID=$GCP_PROJECT_ID`, `GCP_REGION=asia-southeast2`,
...
workflow_settings.yaml                 # $GCP_PROJECT_ID / asia-southeast2 / dwh
...
### 8.3 Path C — `--workflows` selector (migrate only some workflows)
...
> Note: the selector actually used below (`$WORK_DIR/selector.json`) is a
> scratch fixture written outside the repository and is **not committed**, so
> this sub-section is not reproducible as written from a clean checkout; the
> committed `extract/sample_workflows_selector.json` is the equivalent sample.

```bash
uv run migrate/migrate.py all --extract-dir data/extract \
  --workflows $WORK_DIR/selector.json
```
...
### 8.4 Final data check (both automatic paths, identical to the manual paths)
...
Integration test (`$WORK_DIR/test_resume.py`, against the `dataworks-mysql`
container; test DB `dataworks_extract_test` dropped afterwards):

> Note: `$WORK_DIR/test_resume.py` is a scratch fixture written outside the
> repository and is **not committed**, so this step is not reproducible as
...
| **Review items** (3, as of the 2026-08-18 run) | `quality_check` [PYODPS] — not auto-generated | Lightweight procedural Python, needs Composer PythonOperator |
...
> Superseded: the later 2026-08-19 run records 7 review items for this sample
> (4 partition + 2 PYODPS + 1 DIDE_SHELL) — see
> [section 11.3](#end-to-end-generation-testendtoendgeneration).
...
was added, only the 3 inherent pipeline-limit items remain (2026-08-18 run).
...
v3.1 guide vs. code audit. The test suite (`code/tests/test_migrate.py`) covers
all fixed gaps and regression-proofs the existing functionality; for the test
count recorded for this run see the [summary table](#1-summary) and
[section 11.2](#112-test-results).
...
**As of the 2026-08-19 run: 51/51 passed.** Lint: `uv run ruff check migrate/ tests/ classify/` → **1 pre-existing issue (DTZ001 in gcp/dags/), all new code clean!**
...
- **Complex ecommerce**: 4 models, 2 declarations, 7 review items (4 partition + 2 PYODPS + 1 DIDE_SHELL), as of the 2026-08-19 run. This count supersedes the 3 review items recorded for the earlier 2026-08-18 run in [section 10](#10-complex-workflow--sample_flowspec_complex_ecommercejson).
...
[Index](../../README.md) · [Testing overview](../README.md)
