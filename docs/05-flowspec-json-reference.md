...
| `"OfflineWorkflow"` | Manual / ad-hoc workflow (no schedule) |
...
| `cron` | string | `"00 02 00 * * ?"` | Cron: **sec min hour day month weekday**. `00 02 00 * * ?` = daily 02:00 |
...
> `migrate.py` strips the seconds field (`00 02 00 * * ?` → `0 2 0 * * *`) for
> Airflow and passes `timezone` to the DAG.
...
> resolve instead of guessing — see appendix A of docs/06.
...
| command | Engine | Case study? | GCP target (docs/02) | Toolkit behaviour |
| --- | --- | --- | --- | --- |
| `VIRTUAL` | None (no-op) | ✅ `workshop_start` | Airflow `EmptyOperator` | No SQL emitted |
| `ODPS_SQL` | MaxCompute SQL | ✅ both `ddl_*` and `insert_*` | Dataform `.sqlx` | Translated ODPS → GoogleSQL |
| `PYODPS` | MaxCompute Python | — | Dataflow / Composer Python | SQL-ish subset pushed down; complex code flagged **TRIAGE** |
| `DATAX` | Offline sync | — | Dataflow / load job | Captured for review |
| `DIDE_SHELL` / `SHELL` | Shell | — | Composer `BashOperator` | Audited individually |
| `WORKFLOW_DAG` | Nested workflow | — | Sub-DAG / nested composition | Recursively processed |
...
generator treats unknown/unsupported commands conservatively: it writes them to
`review/*.md` and **stops the pipeline** rather than guessing.
...
3. **workflows[0].trigger**: `cron = "00 02 00 * * ?"` in `Asia/Jakarta` →
   every day at 02:00 Jakarta time.
