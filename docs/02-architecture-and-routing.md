...
Orchestration and metadata live in **different API versions, different SDKs,
different endpoints**. You must instantiate **two clients**.

| Purpose | API version | Example operations |
| --- | --- | --- |
| Orchestration (workflows, nodes, code, dependencies) | `2024-05-18` | `ListWorkflows`, `GetWorkflow`, `ListNodes`, `GetNode`, `GetWorkflowDefinition` |
| Table metadata (DDL: columns, types, partitions) | `2020-05-18` | `GetMetaTableColumn`, `GetMetaTablePartition`, `GetMetaTableBasicInfo` |

**Known bug in the original draft:** the schema function called
`get_meta_table_column()` on the `2024-05-18` client. That operation only exists
on the `2020-05-18` API, so every schema call would fail. Build a separate
`2020-05-18` client for all Data Map / metadata calls.

> Note: `GetMetaTablePartition` supports **MaxCompute and EMR** engine tables only.
...
**Next:** [03 · Migration steps](03-migration-steps.md)
